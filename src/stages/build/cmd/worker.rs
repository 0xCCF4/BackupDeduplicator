    ...
    let path = match path {
        ...
        Err(e) => {
            ...
            worker_publish_result_or_trigger_parent(
                id,
                false,
                worker_create_error(job.target_path.clone(), 0, 0),
                job,
                result_publish,
                job_publish,
                arg,
...
/// * `job_publish` - The channel to publish new jobs to.
/// * `arg` - The argument for the worker thread.
fn worker_publish_result_or_trigger_parent(
    id: usize,
    cached: bool,
    result: BuildFile,
    job: BuildJob,
    ...
    match job.parent {
        Some(parent) => {
            ...
            worker_publish_result(
                id,
                result_publish,
                JobResult::Intermediate(JobResultContent {
        ...
        None => {
            worker_publish_result(
                id,
                result_publish,
                JobResult::Final(JobResultContent {
                    already_cached: cached,
                    content: result,
                }),
            );
    ...
    match Arc::into_inner(parent_job) {
        Some(parent_job) => {
            trace!(
                "[{}] finished last child of parent {:?}",
                id,
                parent_job.target_path
            );
